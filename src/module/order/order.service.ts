...
export class OrderService {
  ...
  async findOpenOrdersForVendors(daysAgo: number): Promise<Order[]> {
    ...
    console.log(
      ...
      estimatedDaysAgo,
      ':: ',
      estimatedDaysAgo1 > estimatedDaysAgo,
      OptionType.ACCEPTED,
  ...
  async getOrderByStatusAndRange(
    page,
    limit: number,
    status: OptionType,
    start,
  ...
  async listVendorOrders(
    ...
    try {
      const skip = (page - 1) * limit;
      const { startDate, endDate } =
        this.utilityService.calculatePreviousDate(daysRange);
  ...
  async completeProcessing(vendorID, orderID: string, itemIDs: string[]) {
    try {
      ...
      const shipmentData: Shipment = {
        ...
        itemsCost: matchingItems.reduce(
          (a, b) =>
            a.newPrice
              ? a.newPrice * a.quantity
              : a.salesPrice * a.quantity + b.newPrice
                ? b.newPrice * b.quantity
                : b.salesPrice * b.quantity,
          0,
        ),
                    ...
                    $eq: [
                      {
                        $size: {
                          $filter: {
                            ...
                            as: 'item',
                            cond: {
                              $ne: ['$$item.status', OptionType.SHIPPED],
                            },
                          },
                        },
                      },
  ...
  async getMonthlyOrderDataByVendorID(vendorID, startDate, endDate) {
    console.log('DTO== ', vendorID, startDate, endDate);
    ...
    try {
      const result = await this.orderModel.aggregate([
        {
          $match: {
            ...
            'items.vendorID': vendorID, // Filter by specific vendorID
            createdAt: { $gte: new Date(startDate), $lte: new Date(endDate) }, // Filter by time frame
          },
        },
        {
          $unwind: '$items', // Flatten the items array to access vendorID
        },
        {
          $group: {
            _id: {
            ...
            totalOrdersByVendorID: {
              $map: {
                ...
                in: {
                  ...
                  isSuccessful: {
                    $cond: [
                      { $eq: ['$$vendorData.status', 'DELIVERED'] }, // Check for successful status
                      '$$vendorData.count',
                      0,
                    ],
                  },
        ...
        },
        {
          $project: {
            month: 1,
            year: 1,
            totalOrdersByVendorID: 1,
            totalSales: {
              $sum: {
                $map: {
                  input: '$totalOrdersByVendorID',
                  as: 'vendorData',
                  in: '$$vendorData.isSuccessful',
                },
              },
            },
          },
      ...
      return result.map((item) => ({
        ...
        totalOrdersByVendorID: item.totalOrdersByVendorID,
  ...
  async getVendorOrderCountsByMonth(
    vendorID,
    startDate: string,
    endDate: string,
  ) {
    return this.orderModel.aggregate([
      {
        $match: {
          ...
          status: {
            $in: ['ACCEPTED', 'WAREHOUSED', 'SHIPPED', 'DELIVERED'],
          },
        },
      },
      {
              ...
              $cond: [
                {
                  $in: [
                    '$_id.status',
                    ['ACCEPTED', 'WAREHOUSED', 'SHIPPED', 'DELIVERED'],
                  ],
                },
                '$count',
                0,
  ...
  getFulfilledVsAcceptedOrders = async (
    vendorID: string,
    startDate: string,
    endDate: string,
  ) => {
    return this.orderModel.aggregate([
        ...
        $group: {
          ...
          fulfilled: {
            $sum: {
              $cond: [
                {
                  $in: ['$_id.status', ['WAREHOUSED', 'SHIPPED', 'DELIVERED']],
                },
                '$count',
                0,
              ],
      ...
      { $sort: { year: 1, month: 1 } }, // Sorting chronologically
    ]);
  };

  async getOrderCountsByMonth(startDate: string, endDate: string) {
    return this.orderModel.aggregate([
      {
        $match: {
          updatedAt: {
